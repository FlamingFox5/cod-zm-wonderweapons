...
SWEP.CustomBulletCallback = function(ply, tr, dmg)
	if SERVER and nzombies then
		local ent = tr.Entity
		if nzombies and IsValid(ent) and nzRound and nzRound.GetZombieHealth and ent:IsValidZombie() and ply:IsPlayer() then
...
function SWEP:AltAttack()
	...
	if not nzombies and self:GetNuking() then return end
...
function SWEP:CanSecondaryAttack()
	...
	if self:GetPrimaryClipSize(true) > 0 and (nzombies and self:Clip1() or self:Ammo1()) < self:GetStatL("Secondary.AmmoConsumption") then
		return false
	end
...
function SWEP:Think2(...)
	...
	if self:GetGlowRatio() > 0 and status ~= TFA.Enum.STATUS_SHOOTING then
		self:SetGlowRatio(math.Approach(self:GetGlowRatio(), 0, FrameTime()))
		//self.Glow:SetFloat("$emissiveblendstrength", self:GetGlowRatio())
	end

	if status == TFA.Enum.STATUS_HOLSTER and statusend then
		self:SetGlowRatio(0)
	end
	...
	if status == TFA.Enum.STATUS_BASHING then
		if self:GetStatusProgress() >= 0.5 and not self:GetHasNuked() then
			...
			if !nzombies and SERVER then
				timer.Create("overkill_reset"..self:EntIndex(), self.NukeCooldown, 1, function()
		...
		if ply:KeyDown(IN_ATTACK) and self:CanSpinUp() then
			if not self:GetIsSpinning() then
				if IsFirstTimePredicted() then
					self:EmitSoundNet("TFA_BO4_OVERKILL.SpinStart")
				end
			end
			...
			if l_CT() > self:GetNextPrimaryFire() or self.Primary.Automatic then
				if self:GetIsSpinning() then
					if IsFirstTimePredicted() then
						self:EmitSoundNet("TFA_BO4_OVERKILL.SpinStop")
					end
				end
...
function SWEP:PostPrimaryAttack()
	self:SetGlowRatio(math.Approach(self:GetGlowRatio(), 3, 0.02))
	//self.Glow:SetFloat("$emissiveblendstrength", self:GetGlowRatio())
end
