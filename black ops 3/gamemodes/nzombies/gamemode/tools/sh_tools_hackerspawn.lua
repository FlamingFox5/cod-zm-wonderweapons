nzTools:CreateTool("hacker_spawnpoint", {
	...
	interface = function(frame, data)
		...
		valz["UtilityList"] = data.utilitylist or {}
		...
		function DProperties.CompileData()
			...
			data.utilitylist = valz["UtilityList"] or {}
...
if SERVER then
	...
	nzMapping:AddSaveModule("HackerSettings", {
		savefunc = function()
			local data = {
				utilitylist = nzHackerDevice.UtilityList
			}
			return data
		end,
		loadfunc = function(data)
	...
	hook.Add("OnRoundInit", "NZ.BO3WW.FOX.Hacker.ResetButtons", function()
	...
	hook.Add("OnRoundEnd", "NZ.BO3WW.FOX.Hacker.ResetButtons", function()
		for _, ply in ipairs(player.GetAll()) do
			ply.HackerHacks = nil
			ply.HackerDevicePAP = nil
		end
	...
	hook.Add("OnDoorUnlocked", "NZ.BO3WW.FOX.Hacker.ActivateButtons", function(ent, link, _, ply)
		for k, v in pairs(ents.FindByClass("nz_bo3_hackerbutton")) do
