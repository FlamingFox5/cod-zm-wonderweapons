...
function ENT:EntityCollide(trace)
	...
	TFA.WonderWeapon.DoDeathEffect(hitEntity, self:GetUpgraded() and "BO3_Octobomb_Upgraded" or "BO3_Octobomb", math.Rand(4, 6))
...
function ENT:Think()
	if SERVER then
		if ( !self.NextScream or self.NextScream < CurTime() ) and !self:GetDying() then
			self:EmitSound("TFA_BO3_ARNIE.AttackVox")
			self.NextScream = CurTime() + math.Rand(3, 5)
		end
...
function ENT:PreExplosionDamage( hitEntity, explosionTrace, damageinfo )
	...
	TFA.WonderWeapon.DoDeathEffect(hitEntity, "BO3_Octobomb", math.Rand(4, 6), self:GetUpgraded())
end
