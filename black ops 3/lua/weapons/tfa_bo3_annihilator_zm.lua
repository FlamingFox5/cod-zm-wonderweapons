...
SWEP.Primary.Damage = nzombies and 11500 or 1150
...
SWEP.NZSpecialistResistanceData = {
	Types = bit.bor(DMG_BULLET, DMG_AIRBOAT, DMG_BUCKSHOT, DMG_BLAST, DMG_BLAST_SURFACE, DMG_SONIC, DMG_VEHICLE),
...
SWEP.CustomBulletCallback = function(ply, trace, damageinfo)
	local hitEntity = trace.Entity
	if not SERVER then return end
	if IsValid(hitEntity) and IsValid(ply) and ply:IsPlayer() then
		if nzombies and hitEntity.NZBossType then
			damageinfo:SetDamage(math.max(2400, hitEntity:GetMaxHealth() / 8))
			damageinfo:ScaleDamage(math.Round(nzRound:GetNumber()/8))
		end

		if hitEntity:IsPlayer() or hitEntity:IsNextBot() or hitEntity:IsNPC() then
			if !TFA.WonderWeapon.ShouldDamage(hitEntity, ply, damageinfo:GetInflictor()) then return end
			...
			//damageinfo:SetDamageForce(((damageinfo:GetDamagePosition() - ply:GetShootPos()):GetNormalized() * 20000) + ent:GetUp()*3000)

			if hitEntity.GetBloodColor and DontBleed[hitEntity:GetBloodColor()] then return end

			hitEntity:EmitSound("TFA_BO3_ANNIHILATOR.Lfe")

			TFA.WonderWeapon.DoDeathEffect(hitEntity, "BO3_Annihilator_Gib")
		end
	end
end
