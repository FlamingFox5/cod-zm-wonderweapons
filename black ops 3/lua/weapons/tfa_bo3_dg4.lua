...
SWEP.Primary.RPM = 90
SWEP.Primary.Damage = nzombies and 115 or 125
...
SWEP.Secondary.Damage = nzombies and 115 or 50
...
function SWEP:PreDrawViewModel(vm, ...)
	...
	if IsValid(ply) and dlight_cvar:GetBool() and DynamicLight then
...
function SWEP:DrawWorldModel(...)
	...
	if dlight_cvar:GetInt() == 1 and DynamicLight then
		self.DLight = self.DLight or DynamicLight(self:EntIndex(), false)
		if self.DLight then
			self.DLight.pos = IsValid(ply) and (ply.GetShootPos and ply:GetShootPos() or ply:EyePos()) - (ply:OBBCenter() * 1/3) or self:GetPos()
			self.DLight.r = 45
			self.DLight.g = 165
			self.DLight.b = 255
			self.DLight.decay = 1000
	...
	if SERVER then
		...
		for k, v in pairs(ents.FindInSphere(ply:GetShootPos(), range)) do
			...
			if v.GetPhysicsObject then
				...
				if IsValid( phys ) and phys:IsMotionEnabled() and ( v:IsSolid() or v:GetCollisionGroup() == COLLISION_GROUP_DEBRIS ) then
			...
			damage:SetDamageForce(vector_up*20000 + (epos - Vector(position[1], position[2], epos[3] - 1)):GetNormalized() * 25000)
			...
			local hitAlive = ( v:IsNPC() or v:IsPlayer() or v:IsNextBot() or v:IsRagdoll() or v:IsVehicle() )
			if hitAlive then
				TFA.WonderWeapon.DoDeathEffect(v, "BO3_Ragnarok", math.Rand(3,6))
			end
...
function SWEP:CanSecondaryAttack()
	...
	return tr.HitWorld and self:CanPrimaryAttack() and (nzombies and self:Clip1() or self:Ammo1()) >= self:GetStatL("Primary.AmmoConsumption")
end
