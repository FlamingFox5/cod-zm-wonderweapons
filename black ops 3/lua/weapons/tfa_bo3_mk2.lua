...
SWEP.Primary.Damage = nzombies and 600 or 65
SWEP.Primary.Knockback = nzombies and 35 or 10
...
{ ["time"] = 75 / 30, ["type"] = "lua", value = function(self) self.Bodygroups_W = {[1] = 0} end, client = true, server = true},
...
function SWEP:PostPrimaryAttack()
	...
	if self:VMIV() and IsFirstTimePredicted() then
		if self.Ispackapunched then
			ParticleEffectAttach("bo3_mk2_arc_2", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 4)
			ParticleEffectAttach("bo3_mk2_arcb_2", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 5)
		else
			ParticleEffectAttach("bo3_mk2_arc", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 4)
			ParticleEffectAttach("bo3_mk2_arcb", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 5)
		end
	end
