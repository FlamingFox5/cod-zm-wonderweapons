...
SWEP.Primary.Damage = nzombies and 115 or 75
...
function SWEP:TertiaryAttack()
	...
	if sp and SERVER then
		self:CallOnClient('MicrowaveCylinderDamage')
	end
end
...
function SWEP:MicrowaveCylinderDamage()
	...
	local PlayerSizeHulls = {}
	if SERVER then
		PlayerSizeHulls[HULL_HUMAN] = true
		PlayerSizeHulls[HULL_WIDE_HUMAN] = true
		PlayerSizeHulls[HULL_LARGE] = true
		PlayerSizeHulls[HULL_LARGE_CENTERED] = true
		PlayerSizeHulls[HULL_MEDIUM_TALL] = true
	end
	...
	for i, ent in pairs(ents.FindInSphere(view_pos, outer_range*1.1)) do
		...
		if TFA.WonderWeapon.HasStatus( ent, "BO3_Wavegun_Cook" ) then continue end
		if not (ent:IsNPC() or ent:IsNextBot() or ent:IsPlayer() or ent:IsRagdoll() or CLIENT_RAGDOLLS[ent:GetClass()]) then continue end
		...
		if CLIENT or ent:IsRagdoll() then
			if CLIENT_RAGDOLLS[ ent:GetClass() ] or ent:IsRagdoll() then
				local owner = ent:GetRagdollOwner()
				if !TFA.WonderWeapon.HasDeathEffect( ent, "BO3_Wavegun_Cook" ) and (!IsValid( owner ) or !TFA.WonderWeapon.HasStatus( owner, "BO3_Wavegun_Cook" )) then
					...
					if IsValid( phys ) then
						...
						for i=0, ent:GetPhysicsObjectCount() - 1 do
							...
							if IsValid(phys_i) then
								...
								phys_i:ApplyForceOffset(vector_up*40, phys_i:LocalToWorld(phys_i:GetMassCenter()))
								phys_i:AddAngleVelocity(VectorRand():GetNormalized()*200)
							end
						end
					end
					TFA.WonderWeapon.DoDeathEffect( ent, "BO3_Wavegun_Cook" )
				end
			end
		...
		if nzombies and ent:IsValidZombie() and ent.IsMooSpecial then
			...
			if ( colMaxs[1] < 10 or colMaxs[2] < 10 or colMaxs[3] < 40 )  then
				bTooSmall = true
			end

			local _, boundsMaxs = ent:GetSurroundingBounds()
			if !bTooSmall and ( boundsMaxs[1] < 21 or boundsMaxs[2] < 21 or boundsMaxs[3] < 40 ) then
				bTooSmall = true
			end
		end

		if ent.GetHullType and !PlayerSizeHulls[ent:GetHullType()] and ( sv_shrink_all == nil or !sv_shrink_all:GetBool() ) then
			bTooSmall = true
		end
		...
		timer.Simple( math.Clamp( engine.TickInterval() * delay, 0, 0.3 ), function()
			...
			if ( bBossZombie or bTooSmall ) then
				...
				elseif !traceHit.HitWorld then
					...
					hitDamage:SetInflictor(self)
					hitDamage:SetDamageType(DMG_ENERGYBEAM)
					hitDamage:SetDamageForce(self:GetAimVector()*8000)
				...
				TFA.WonderWeapon.GiveStatus( ent, "BO3_Wavegun_Cook", nzombies and math.Rand(3,4) or math.Rand(1.8,2.4), ply, self )
			end
		end )
	end
end
