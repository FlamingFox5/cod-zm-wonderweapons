...
function SWEP:FireAnimationEvent(pos, ang, event, options, ent)
	if (event == 5001) then
		timer.Simple(0, function()
			...
			if wep.CNewViewModelParticles and wep.CNewViewModelParticles[ "slipgun_glow" ] then return end
...
function SWEP:AttachVMEffects()
	if CLIENT then
		WonderWeapons.StopDrawParticle( self, "slipgun_glow", true )

		self:AddDrawCallViewModelParticle(self.Ispackapunched and "bo3_sliquifier_vm_2" or "bo3_sliquifier_vm", PATTACH_POINT_FOLLOW, 2, true, "slipgun_glow")
	end
end
...
function SWEP:PreDrawViewModel(vm, ...)
	...
	self:AddDrawCallViewModelParticle(self.Ispackapunched and "bo3_sliquifier_vm_2" or "bo3_sliquifier_vm", PATTACH_POINT_FOLLOW, 2, !TFA.Enum.ReloadStatus[status] and !TFA.Enum.HolsterStatus[status], "slipgun_glow")
end

local tpfx_cvar = GetConVar("cl_tfa_fx_wonderweapon_3p")
function SWEP:DrawWorldModel(...)
	self:AddDrawCallWorldModelParticle(self.Ispackapunched and "bo3_sliquifier_3p_2" or "bo3_sliquifier_3p", PATTACH_POINT_FOLLOW, 2, tpfx_cvar:GetBool() and not TFA.Enum.HolsterStatus[self:GetStatus()], "slipgun_glow")
